...
class MineralPlacer(size: Int) : DelayedChunkPlacer(size) {
	...
	override fun place(chunk: Chunk, chunkIndex: Int) {
				...
				for (z in 0..15) {
					...
					if (
						...
						when (random.nextInt(0, 4)) {
							0 -> {
								replaceType = Material.GRANITE; moveX = 0; moveZ = -4
							}
							1 -> {
								replaceType = Material.DIORITE; moveX = 4; moveZ = 0
							}
							2 -> {
								replaceType = Material.ANDESITE; moveX = 0; moveZ = 4
							}
							else -> {
								replaceType = Material.TUFF; moveX = -4; moveZ = 0
							}
						}
		...
		for (y in (OreFix.HEIGHT_LIMIT + 1)..OreFix.GRADIENT_LIMIT) {
			doChunkLayer(y,
				(1 - Util.invInterp(OreFix.HEIGHT_LIMIT.toFloat() - 1f,
					OreFix.GRADIENT_LIMIT + 2f,
					y.toFloat())) * BASE_CHANCE)
		}
